class SnrgSchemePlanningView {
  ...
  make_filters() {
    ...
    this.controls.scheme.get_query = () => ({
      filters: {
        scheme_type: this.options.schemeTypeFilter,
      },
    });
    ...
    return `
      <div class="snrg-scheme-card">
        ...
        <div class="snrg-scheme-table-wrap">
          ${this.options.mode === "category"
            ? this.render_category_customer_table(scheme, tableKey)
            : this.render_amount_customer_table(scheme, tableKey)}
        </div>
      </div>
    `;
  }
  ...
  render_amount_customer_table(scheme, tableKey) {
  ...
  render_customer_rows(scheme, tableKey) {
    ...
    if (!rows.length) {
      const colspan = this.options.mode === "category"
    ...
    return rows.map((row) => (
      this.options.mode === "category"
        ? this.render_category_customer_row(scheme, row)
        : this.render_customer_row(scheme, row)
    )).join("");
  }
  ...
  render_customer_row(scheme, row) {
  ...
  render_category_customer_row(scheme, row) {
    const key = `row-${this.detailKeyCounter++}`;
    this.customerIndex[key] = { scheme, row };
  ...
  open_customer_details(key) {
    ...
    const summaryCards = this.options.mode === "category"
      ? `
          ${categoryCards}
...
window.SnrgSchemePlanningView = SnrgSchemePlanningView;
