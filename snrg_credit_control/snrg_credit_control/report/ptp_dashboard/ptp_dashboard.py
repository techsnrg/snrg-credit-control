...
def execute(filters=None):
    ...
    return _get_columns(), rows, None, _get_chart(rows), _get_summary(rows)
...
def _get_columns():
    return [
        ...
        {"label": "Requested To", "fieldname": "requested_to_employee", "fieldtype": "Link", "options": "Employee", "width": 150},
        ...
        {"label": "Calendar Event", "fieldname": "calendar_event", "fieldtype": "Link", "options": "Event", "width": 140},
...
def _get_summary(rows):
    ...
    committed = sum(flt(row.committed_amount) for row in rows if row.status in active_statuses)
    received = sum(flt(row.received_amount) for row in rows if row.status in active_statuses)
    difference = sum(flt(row.difference_amount) for row in rows if row.status in active_statuses)
